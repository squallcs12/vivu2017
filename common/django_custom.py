...
def HttpJson(obj, status=200):
    return HttpResponse(JSONSerializer().serialize(obj, use_natural_keys=True), content_type="application/json", status=status)
...
class JSONSerializer():
    ...
    def handle_object(self, obj):
        ...
        elif isinstance(obj, int) or isinstance(obj, float) or isinstance(obj, long) or isinstance(obj, tuple):
            self.handle_simple(obj)
